...
void generate(struct generator *gen)
{
    ...
    if (has_custom_tokens) {
        ...
        output_line(out, "    owl_token_func_t tokenize;");
        output_line(out, "    // This pointer will be passed into the tokenize function.");
        output_line(out, "    void *tokenize_info;");
    }
    output_line(out, "};");
    output_line(out, "struct owl_tree *owl_tree_create_with_options(struct owl_tree_options options);");
    output_line(out, "");
    ...
    output_line(out, "");
    output_line(out, "    // The file passed to owl_tree_create_from_file was NULL.");
    output_line(out, "    ERROR_INVALID_FILE,");
    output_line(out, "");
    output_line(out, "    // The options passed to owl_tree_create_with_options weren't valid --");
    ...
    if (has_custom_tokens) {
        output_line(out, "");
        output_line(out, "enum owl_token_type {");
    ...
    if (has_custom_tokens) {
        output_line(out, "    owl_token_func_t custom_tokenize;");
    ...
    if (has_custom_tokens) {
        ...
        output_line(out, "        return false;");
        output_line(out, "    struct owl_token t = tree->custom_tokenize(text, tree->custom_tokenize_info);");
    ...
    output_line(out, "    }");
    output_line(out, "}");
    output_line(out, "static void parse_string(struct owl_tree *tree, const char *string) {");
    ...
    output_line(out, "}");
    output_line(out, "struct owl_tree *owl_tree_create_from_string(const char *string) {");
    output_line(out, "    return owl_tree_create_with_options((struct owl_tree_options){ .string = string });");
    output_line(out, "}");
    output_line(out, "struct owl_tree *owl_tree_create_from_file(FILE *file) {");
    output_line(out, "    if (!file)");
    output_line(out, "        return owl_tree_create_with_error(ERROR_INVALID_FILE);");
    output_line(out, "    return owl_tree_create_with_options((struct owl_tree_options){ .file = file });");
    output_line(out, "}");
    output_line(out, "struct owl_tree *owl_tree_create_with_options(struct owl_tree_options options) {");
    output_line(out, "    if (!options.file == !options.string)");
    output_line(out, "        return owl_tree_create_with_error(ERROR_INVALID_OPTIONS);");
